...
export default function PostCard({ post, onDeleteSuccess, showMenu = true }: PostCardProps) {
  const router = useRouter();
  const [isLiked, setIsLiked] = useState<boolean>(Boolean(post.liked || post.isLiked || false));
  ...
  const [isSaved, setIsSaved] = useState<boolean>(Boolean(post.saved || false));
  const [isPinned, setIsPinned] = useState<boolean>(Boolean(post.isPinned || false));
  ...
  const handleSaveToggle = async () => {
    ...
    try {
      ...
      if (res.data?.success !== undefined && !res.data.success) {
        setIsSaved(isSaved);
      } else if (res.data?.isSaved !== undefined) {
