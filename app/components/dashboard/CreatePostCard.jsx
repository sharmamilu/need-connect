...
export default function CreatePostCard({ onSubmit }) {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      // Show whatever the response is from POST api as requested by the user
      Alert.alert(
        "Post API Success Response",
  ...
  const isBackgroundActive =
    background && background.id !== "none" && images.length === 0 && selectedTemplateId === "custom";
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Select a template to craft highly professional content</Text>
        </View>
          ...
          {POST_TEMPLATES.map((t) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.templateCard,
                  isActive && { borderColor: t.color, backgroundColor: t.bg },
                ]}
                activeOpacity={0.8}
                onPress={() => setSelectedTemplateId(t.id)}
              >
                <View style={[styles.templateIcon, { backgroundColor: isActive ? "#fff" : t.bg }]}>
                  <Feather name={t.icon} size={16} color={t.color} />
                </View>
                <Text style={[styles.templateCardTitle, isActive && { color: t.color }]}>
                  {t.title}
                </Text>
              </TouchableOpacity>
            );
        ...
        {/* ACTIVE TEMPLATE INTRO */}
        <View style={styles.activeTemplateBox}>
          <Text style={styles.activeTemplateTitle}>{activeTemplate.title} Template</Text>
          <Text style={styles.activeTemplateDesc}>{activeTemplate.description}</Text>
        </View>
